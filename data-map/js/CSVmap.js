...
function setup() {
  ...
  L.tileLayer('https://api.maptiler.com/maps/voyager/{z}/{x}/{y}.png?key=6e5dj7C0TZ5PY4pS6DXo', {}).addTo(myMap)
  ...
  var greenLeaf = new LeafIcon({
      iconUrl: 'assets/tea-leaf-05.png'
    }),
    blueLeaf = new LeafIcon({
      iconUrl: 'assets/tea-leaf-06.png'
    }),
    purpleLeaf = new LeafIcon({
      iconUrl: 'assets/tea-leaf-07.png'
    }),
    pinkLeaf = new LeafIcon({
      iconUrl: 'assets/tea-leaf-08.png'
    });
  ...
  var homeMarker = L.marker([41.91113281, -87.68097687], {
      icon: homeIcon
    })
    .bindPopup(popup)
    .addTo(myMap);
  ...
  for (let i = 0; i < countries.getRowCount(); i++) {
    ...
    if (tea.includes("oolong")) {
      oolongTeaArray.push(L.marker([lat, long], {
          icon: purpleLeaf
        })
        .bindPopup('<div class="location-popup"><div class="location-content"><h1>' + country + '</h1><p>Tea Trade Value: ' + tradeValue + '</p>' + extra + '</div><div class="location-img"><img src= "assets/images/' + locationImage + '"/></div></div>'));
    }
  ...
  L.control.layers(null, teaOverlay, {
